...
**2026-09-16 07:16（Asia/Shanghai）每日开启相位报「22/22 未落地」——后续证据表明极可能是旧代码误报，开启实际已生效。**

- 审计证据：`data/state.json` → `batches.瑾漂亮潮流服饰.2026-09-16.runs[0]`，
  ...
  `data/audit.jsonl` 同批次（`plan` → `retry` → `abort`/`chengfang-enable-batch`/`action`）。
- **当时运行的代码版本**：`1996e15`（2026-09-15 13:50 提交，**早于第十六轮** `0972257`）。
  旧版 `enablePageTargets`：点击 → 恢复管理页 → **立即单次回读**（无落地轮询）→ 未落地即判失败 →
  重试**再点一次** → 3.4 秒后再次立即回读 → 仍读不到落地 → abort「开关仍关闭」。
  时间线：`plan(23:16:20Z) → retry(23:16:48Z)` 27.3 秒、`retry(23:16:48Z) → abort(23:16:51Z)` 仅 3.4 秒。
- **反证（09:43 只读探针，`evidence/enable-failure-probe/current-state.json`）**：
  第 1 页可见 10 行中，**报失败清单里的 9 个 ID 全部已是开启态**，`stillClosed=0`，
  托管计划开关亦为开启（`already_enabled`，07:16:01 已核验）；另 13 个目标不在第 1 页（探针未切 100/页，未逐页核验）。
- **07:16 之后审计中再无任何 enable/pause 动作** → 最合理解释：开启请求当时已受理，
  平台异步落地慢于两次立即回读（项目注释早已记录「实测 27 秒后仍未落地、稍后全部生效」），
  旧代码把「尚未落地」误判为「开启未生效」。13 个未逐页核验的目标不能 100% 排除人工干预，但无任何此类迹象。
- **与本轮修复的关系（重要）**：这正是第十六轮引入 `confirmLanding` 有界落地轮询、
  第十七轮 `landingStateTrustworthy` 闸要消灭的两类问题——
  ①立即回读误报失败；②把"未知状态"当作重发依据（旧代码在落地未确认时**又点了一次**，
  本次侥幸无反向后果，但批量开启重复提交是真实风险）。
  修复后：轮询最长 `execution.readbackTimeoutMs=30s` 等待落地；状态不可信时禁止重发。
- 本轮**未**为验证此事发起任何真实广告动作；下一次真实机会为 **2026-09-17 07:00**（注意：若 09-17 07:00
  读取到对象已全部开启，属幂等跳过，只证明调度与回读有效，不证明点击路径）。
- 状态文件中 09-16 的 `unknown`/`allEnabledConfirmed=false` 记录**保留不改**：
  它如实反映"当时未确认"，诚实记录正是设计行为；后续轮次以当天真实读取为准。
