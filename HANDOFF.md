...
## 0.8 生产实测观测：2026-09-16 07:16 每日开启相位报「22/22 未落地」——极可能是旧代码误报，开启实际已生效
...
- 审计证据：`data/state.json` → `batches.瑾漂亮潮流服饰.2026-09-16.runs[0]`：
  ...
  `data/audit.jsonl` 同批次有 `plan → retry → abort`。
- **当时运行的代码**：`1996e15`（2026-09-15 13:50，**早于第十六轮**）。旧版开启路径是
  点击 → **立即单次回读**（无落地轮询）→ 未落地即判失败 → 重试**再点一次** → 3.4 秒后再次立即回读 → abort。
  时间线：`plan(23:16:20Z) → retry(23:16:48Z)` 27.3 秒；`retry → abort(23:16:51Z)` 仅 3.4 秒。
- **反证（09:43 只读探针，`evidence/enable-failure-probe/current-state.json`）**：
  第 1 页可见 10 行中**报失败清单里的 9 个 ID 已全部开启**、`stillClosed=0`、托管开关亦开启
  （07:16:01 即核验 `already_enabled`）；另 13 个目标不在第 1 页（探针未切 100/页，未逐页核验）。
  07:16 后审计中**再无任何 enable/pause 动作**。
- **结论**：最合理解释是开启请求当时已受理、平台异步落地慢于两次立即回读
  （项目注释早有「实测 27 秒后仍未落地、稍后全部生效」），旧代码把「尚未落地」误判为「开启未生效」。
  这正是第十六/十七轮修复要消灭的问题：①立即回读误报；②落地未确认就**再点一次**（重复提交风险）。
  修复后：最长 30s 有界轮询等落地；`landingStateTrustworthy` 闸禁止据不可信状态重发。
- 状态文件 09-16 的 `unknown` 记录**保留不改**（如实反映"当时未确认"，是设计行为）；
  09-17 07:00 若读到全部已开启，属幂等跳过，只证明调度与回读有效。
- 本轮**未**为验证此事发起任何真实广告动作。
